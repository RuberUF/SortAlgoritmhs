...
def quick_sort(arr, order_type='asc'):
    ...
    left = [x for x in arr if (order_type == 'asc' and x < pivot) or (order_type == 'desc' and x > pivot)]
    middle = [x for x in arr if x == pivot]
    right = [x for x in arr if (order_type == 'asc' and x > pivot) or (order_type == 'desc' and x < pivot)]

    return quick_sort(left, order_type) + middle + quick_sort(right, order_type)
...
def inorder_traversal(node, result, ascending=True):
    if node is not None:
        if ascending:
            inorder_traversal(node.left, result, ascending)
            result.append(node.value)
            inorder_traversal(node.right, result, ascending)
        else:
            inorder_traversal(node.right, result, ascending)
            result.append(node.value)
            inorder_traversal(node.left, result, ascending)
...
def binary_tree_sort(data, order_type='ascendente'):
    if not data:
        return []
    ...
    sorted_data = []
    inorder_traversal(root, sorted_data)

    if order_type == 'descendente':
        sorted_data.reverse()
        
    return sorted_data

def inorder_traversal(node, sorted_data):
    if node:
        inorder_traversal(node.left, sorted_data)
        sorted_data.append(node.value)
        inorder_traversal(node.right, sorted_data)
